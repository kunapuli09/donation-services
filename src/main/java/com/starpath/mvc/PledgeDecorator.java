...
public class PledgeDecorator implements DisplaytagColumnDecorator {
	public Object decorate(Object columnValue, PageContext pageContext,
		...
		String sValue = null;
		  if(pageContext.getPage().toString().toLowerCase().indexOf("searchusers")>0)
			{
				sValue = "<a class=\"tablelink\" href=\"addPledge.htm?&userid="+columnValue.toString()+"\">Add</a>";	;
			}
		  else if(pageContext.getPage().toString().toLowerCase().indexOf("searchpledges")>0)
			{
				sValue = "<a class=\"tablelink\" href=\"editPledge.htm?&id="+columnValue.toString()+"\">Disable</a>";	;
			}
		return sValue;
