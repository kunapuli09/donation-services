...
public class PaymentDetailDecorator implements DisplaytagColumnDecorator {
	public Object decorate(Object columnValue, PageContext pageContext,
		...
		String sValue = null;
		  if(pageContext.getPage().toString().toLowerCase().indexOf("searchusers")>0)
			{
				sValue = "<a class=\"tablelink\" href=\"editPayment.htm?&id="+ columnValue.toString()+"\">Add</a>";
			}
		  else if(pageContext.getPage().toString().toLowerCase().indexOf("searchpaymentdetail")>0)
			{
				sValue = "<a class=\"tablelink\" href=\"editPayment.htm?&id="+ columnValue.toString()
				+"\">Edit</a>";	
			}
		return sValue;
